...
public class BenchmarkImpl implements Benchmark {
	...
	public void start() {
		...
		for (int messages : rampUpMessages) {
			...
			doBenchmark("Vertx 3 TCP Server", results, messages, new AsyncRestTemplateTester(port_vertx3_tcp, host, servletAsyncPath));

			doBenchmark("Jetty Async Servlet", results, messages, new AsyncRestTemplateTester(port_jetty, host, servletAsyncPath));
			doBenchmark("Jetty Sync Servlet", results, messages, new AsyncRestTemplateTester(port_jetty, host, servletSyncPath));

			doBenchmark("SpringBoot Jetty Async", results, messages, new AsyncRestTemplateTester(port_spring_boot_jetty, host, servletAsyncPath));
			doBenchmark("SpringBoot Jetty Sync", results, messages, new AsyncRestTemplateTester(port_spring_boot_jetty, host, servletSyncPath));

			doBenchmark("SpringBoot Tomcat Async", results, messages, new AsyncRestTemplateTester(port_spring_boot_tomcat, host, servletAsyncPath));
			doBenchmark("SpringBoot Tomcat Sync", results, messages, new AsyncRestTemplateTester(port_spring_boot_tomcat, host, servletSyncPath));
